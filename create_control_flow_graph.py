...
def create_control_flow_graph(opcodes_json, G):
    #################建造控制流图#######################
    dfs(opcodes_json, G, 0)  # 真正用于建造控制流图的地方, 使用递归的算法，其中从0开始走
    #################建造控制流图#######################
    for node in G:
...
def dfs(opcodes_json, G, now):
    ...
    if G[now].node_type == 'HEX' and G[now].node_content == 'NULL':  # 这种节点是并不需要连接控制流的，作为孤立节点即可。
        ...
        if len(G[now].cfg_parent) == 1 and G[now].cfg_parent[0].node_type.startswith("PUSH"):
            ...
            if G[next_index].node_type == "JUMPDEST":  # 必须是跳跃点才能进行跳跃
        ...
        if not G[now + length + 1].cfg_parent:  # 只有数组不为空的时候，才能往里面添加控制流，因为这是一条绝路，只能走一次。
        ...
        if not G[now].cfg_edge:  # 只有数组不为空的时候，才能往里面添加控制流，因为这是一条绝路，只能走一次。
        ...
        if not G[now + 1].cfg_parent:  # 只有数组不为空的时候，才能往里面添加控制流，因为这是一条绝路，只能走一次。
